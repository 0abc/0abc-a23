...
function resizeDiplomacyDialog()
{
  ...
  let widthOffset = g_ResourceData.GetCodes("tributable").length * (tribSize.right - tribSize.left) / 2;
...
{
  let resCodes = g_ResourceData.GetCodes("tributable");
  let r = 0;
...
function resizeTradeDialog()
{
  ...
  let length = Math.max(g_ResourceData.GetCodes("tradable").length, g_ResourceData.GetCodes("barterable").length);
...
function openTrade()
{
  ...
  let button = {};
  let resTradeCodes = g_ResourceData.GetCodes("tradable");
  let resBarterCodes = g_ResourceData.GetCodes("barterable");
...
{
  g_BarterSell = g_ResourceData.GetCodes("barterable")[0];
}
...
function updateBarterButtons()
{
  ...
  if (canBarter)
    g_ResourceData.GetCodes("barterable").forEach((resCode, i) => {
