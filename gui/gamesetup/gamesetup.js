...
/**
 * Highlight the "random" dropdown list item.
 */
var g_ColorRandom = "orange";

/**
 * Colour for regular dropdown list items.
 */
var g_ColorRegular = "white";

/**
 * Colour for "Unassigned"-placeholder item in the dropdown list.
 */
...
/**
 * All selectable player colours except gaia.
 */
...
/**
 * Containing the colours to highlight the ready status of players,
 ...
 * their colourized, textual representation.
 */
...
/**
 * Array of strings formatted as displayed, including player name.
 */
var g_ChatMessages = [];
 ...
 * colors			 - Optional array of colours to tint the according dropdown items with.
...
var g_PlayerDropdowns = {
	...
	"playerColorPicker": {
		...
		"select": (selectedIdx, playerIdx) => {
			...
			// If someone else has that colour, give that player the old colour.
...
function onClientJoin(newGUID, newAssignments)
{
	...
	// Assign the client (or only buddies if preferred) to an unused player slot and rejoining players to their old slot.
	if (!isRejoiningPlayer && playername != newAssignments[Engine.GetPlayerGUID()].name)
	{
		...
		if (assignOption == "disabled" || assignOption == "buddies" && g_Buddies.indexOf(splitRatingFromNick(playername).nick) == -1)
			return;
	}
...
/**
 * Set the game attributes the way they were the last time the user left the game setup.
 */
...
function sanitizePlayerData(playerData)
{
	...
	playerData.forEach((pData, index) => {
		...
		// Replace colours with the best matching colour of PlayerDefaults.
		if (g_GameAttributes.mapType != "scenario")
		{
...
{
	if (g_IsNetworked)
		// Remove invalid playerIDs from the servers player assignments copy.
...
function ensureUniquePlayerColors(playerData)
{
	for (let i = playerData.length - 1; i >= 0; --i)
		// If someone else has that colour, assign an unused colour.
...
function launchGame()
{
	...
	for (let i in g_GameAttributes.settings.PlayerData)
	{
		...
		// Count how many players use the chosen name.
...
function swapPlayers(guidToSwap, newSlot)
{
	...
	if (playerID != -1)
	{
		...
		// Swap civilizations and colours if they aren't fixed.
		if (g_GameAttributes.mapType != "scenario")
		{
...
function colorizePlayernameByGUID(guid, username = "")
{
	...
	if (playerID > 0)
	{
		...
		// Enlighten player colour to improve readability.
