...
m.AttackPlan = function(gameState, Config, uniqueID, type, data)
{
	...
	{
		priority = 150;
		this.unitStat.Mounted = { "priority": 1, "minSize": 3, "targetSize": 4, "batchSize": 2, "classes": ["Mounted"],
			"interests": [ ["strength", 1] ] };
		this.neededShips = 1;
	}
...
m.AttackPlan.prototype.assignUnits = function(gameState)
{
	...
	if (this.type == "Raid")
	{
		// Raids are fast attacks: assign all mounted units except some for hunting
		let num = 0;
		for (let ent of gameState.getOwnUnits().values())
		{
			if (!ent.hasClass("Mounted") || !this.isAvailableUnit(gameState, ent))
				continue;
			if (num++ < 2)
				continue;
			ent.setMetadata(PlayerID, "plan", plan);
...
m.AttackPlan.prototype.reassignCavUnit = function(gameState)
{
	for (let ent of this.unitCollection.values())
	{
		if (!ent.position() || ent.getMetadata(PlayerID, "transport") !== undefined)
			continue;
		if (!ent.hasClass("Mounted") || !ent.hasClass("CitizenSoldier"))
			continue;
		...
		for (let check = 0; check < lgth; check++)
		{
			...
			else if (targetId)
			{
				...
				else if (unitTargets[targetId] && unitTargets[targetId] > 0)
				{
					...
					maybeUpdate = true;
				else if (!ent.hasClass("Mounted") && !ent.hasClass("Ranged") &&
			...
			{
				let nearby = !ent.hasClass("Mounted") && !ent.hasClass("Ranged");
